...
2. 不同特权级有什么区别？
  ...
  > 一些 CPU 指令只能在 ring0 运行，低的特权级不能访问高特权级的数据，也不能访问一些中断

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

  > `DPL >= max {CPL, RPL}`，RPL 可以减弱 CPL 的访问权限，但都不能比 DPL 更高。
...
1. 一条指令在执行时会有哪些可能的特权级判断？
  ...
  > 进行访存时需要检查特权级，触发中断和异常时也需要检查

2. 在什么情况下会出现特权级切换？

  > 发生中断，或者地低权级的代码调用高特权级代码

3. int指令在ring0和ring3的执行行为有什么不同？

  > 在 ring0 会使用当前栈，不切换特权级，不压入 `SS:EIP`；在 ring3 会切换到内核栈，切换到 ring0，压入 `SS:EIP`。

4. 如何利用int和iret指令完成不同特权级的切换？

  > 见 lab1 challenge 的实现，需要构造栈结构并进行异常返回
...
1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。
  ...
  > 见 lab1 report，具体略。此时处理器开始使用分段机制。

2. 分析页表的建立过程；

  > 见 lab2 report，具体略。此时处理器 CR3 寄存器开始发挥作用。
...
1. (easy) 自映射的目的是什么？它相比线性映射的好处、不足是什么？
  ...
  > 为了能从 PTE/PDE 的虚地址计算出该页/页表所在页的虚拟地址，以供内核的访问。好处是可以用很简单的办法让其他进程访问某个进程的虚拟地址空间，以及让用户进程访问自己的页表，不足是需要连续的一段虚拟地址空间用于自映射。

2. (easy) Linux和Windows分别采用哪种映射机制（线性映射or自映射）？它们的选择背后有什么原因吗？

  > Linux 使用线性映射，更灵活；Windows 采用自映射机制，更规整，并且方便管理。这与两个系统的设计哲学有关系。
